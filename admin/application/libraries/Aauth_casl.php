...
// to find user id, create sessions and cookies
        ...
        $row = $query->row();
//$encriptpass = $this->User_dashboard_model->getCryptedPassword($upassword);
// if email and pass matches and not banned
        $password = ($this->config_vars['use_password_hash'] ? $pass : $this->getCryptedPassword($pass));
...
// if time is expired
    ...
    public function control($perm_par = FALSE) {
        ...
        $need = $this->is_group_allowed($perm_id);
//        echo $need;
//        exit;
...
//tested
    ...
    public function create_user($email, $pass, $username = FALSE, $usergroup) {
...
//             echo 'aauth_error_password_invalid';
            $valid = FALSE;
        }
...
//             echo 'aauth_error_username_invalid';
            $valid = FALSE;
        }
        if (!$valid) {
//             echo 'valid na';
        ...
        $data = array(
            'email' => $email,
            'pass' => $this->getCryptedPassword($pass), // Password cannot be blank but user_id required for salt, setting bad password for now
...
//            $this->add_member($user_id, $this->config_vars['default_group']);
            $this->add_member($user_id, $usergroup);
...
// Update to correct salted password
            if (!$this->config_vars['use_password_hash']) {
                $data = null;
                $data['pass'] = $this->getCryptedPassword($pass);
                $this->aauth_db->where('id', $user_id);
                $this->aauth_db->update($this->config_vars['users'], $data);
            }
...
//        exit;
    ...
    public function is_in_group($perm_name, $user_id) {
        $this->aauth_db->select('aauth_perm_to_group.group_id');
        $this->aauth_db->from('aauth_perms');
        $this->aauth_db->join('aauth_perm_to_group', 'aauth_perms.id = aauth_perm_to_group.perm_id');
        $this->aauth_db->join('aauth_user_to_group', 'aauth_perm_to_group.group_id = aauth_user_to_group.group_id');
        $this->aauth_db->where('aauth_perms.`name`', $perm_name);
        $data = $this->aauth_db->where('aauth_user_to_group.user_id', $user_id);
...
//        print_r($t);
//        exit;
        if (!empty($t[0]->group_id) || $t[0]->group_id != NULL) {
            return TRUE;
        } else {
            redirect('login');
        }
...
//tested
    ...
    public function user_exist_by_name($name) {
        $aaa = $this->user_exist_by_username($name);
//        echo $aaa;
//        exit;
    }
...
//exit;
        ...
        $query = $this->aauth_db->get($this->config_vars['user_to_group']);
//  log_message('error', $this->aauth_db->last_query());
        if ($query->num_rows() < 1) {
            $data = array(
                'user_id' => $user_id,
                'group_id' => $group_id
...
// deletes from permission table
    ...
    public function is_allowed($perm_par, $user_id = FALSE) {
        ...
        $this->CI->load->helper('url');
        $sss = $this->CI->session->userdata('totp_required');
...
//            if ($this->is_admin($this->CI->session->userdata('id')) OR $this->is_group_allowed($perm_id, $this->config_vars['public_group'])) {
////                   echo '7';
////                    exit;
//                return TRUE;
//            }
////$asd =$this->is_loggedin();
//////echo $asd;
//////exit;
////// if is not login
//            if (!$this->is_loggedin()) {
...
// if variable not set
    ...
    public function can_ilogged_in($username, $password) {
// If email and pass matches
// create session
        $data = array(
            'id' => '1111',
            'username' => $username,
            'email' => 'admin@123@admin.lk',
            'loggedin' => TRUE
        );
...
//			$this->update_activity();
    ...
    public function login_defend($perm_par = FALSE) {

        $session_already_set = $this->CI->session->userdata('username');
    ...
    public function login_web_service($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
...
// Encrypt the password.
        switch ($encryption) {
            ...
            case 'md5-base64' :
                $encrypted = base64_encode(mhash($passMHASH_MD5, $plaintext));
...
// Encrypt the password.
    ...
    public function what_user_can_do($perm_name, $user_id) {
        $this->aauth_db->select('aauth_perm_to_user.perm_save,
aauth_perm_to_user.perm_edit,
aauth_perm_to_user.perm_delete,
aauth_perm_to_user.perm_id,
aauth_perm_to_user.perm_au');
        $this->aauth_db->from('aauth_perm_to_user');
        $this->aauth_db->join('aauth_perms', 'aauth_perm_to_user.perm_id = aauth_perms.id');
        $this->aauth_db->where('aauth_perm_to_user.user_id', $user_id);
        $data = $this->aauth_db->where('aauth_perms.`name`', $perm_name);
//        $query = $this->aauth_db->get();
        $query = $this->aauth_db->get();

        $t = $query->result();
        $shifted = array_shift($t);
        if ($shifted->perm_save == 1) {
            return TRUE;
        } else {
            return FALSE;
        }
        if ($shifted->perm_edit == 1) {
            return TRUE;
        } else {
            return FALSE;
        }
        if ($shifted->perm_delete == 1) {
            return TRUE;
        } else {
            return FALSE;
        }
    ...
    public function is_group_have_permission_for_access($user_id) {

        $this->aauth_db->select('aauth_user_to_group.group_id');
        $this->aauth_db->from('aauth_user_to_group');
        $this->aauth_db->where('aauth_user_to_group.user_id', $user_id);

        $query = $this->aauth_db->get();
        $group = $query->result();
        $shifted_group = array_shift($group);
//        echo $shifted_group->group_id;
//        exit;
        if ($query->num_rows() > 0) {
            $row = $query->row_array();
    ...
    public function getWebServicesConnfig() {
