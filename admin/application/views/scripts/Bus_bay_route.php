<script>
    ...
    jQuery(document).ready(function () {
        masterdata_form_validator = jQuery("#bus_bay_route_form").validate({
            rules: {
                bay_to_route_id: {
                    required: true
                }, bay_id: {
        ...
        var_data_grid_table = jQuery('#data_grid_table').DataTable({
            ...
            "ajax": {
                "url": site_url + "/bus_bay_route/getDatatableRecords",
                "type": "POST"
            },
            "columns": [
			{"data": "bay_id"}, 
			{"data": "route"}, 
			{"data": "bay_to_route_id"}
            ],
            "columnDefs": [
                {
                    "targets": -1,
...
// --------------------- END OF DOCUMENT READY CODE ----------------------------
    ...
    jQuery(document).on('click', '#btn_save', function (event) {
        var is_valid = jQuery("#bus_bay_route_form").valid();
        if (is_valid) {
            save_form();
        } else {
...
// --------------------- END OF EVENT HANDLING CODE ----------------------------
    ...
    function save_form() {
        swal({
            ...
            var data_form = document.getElementById('bus_bay_route_form');
            var formData = new FormData(data_form);
            jQuery.ajax({
                method: 'POST',
                url: site_url + '/bus_bay_route/save',
                dataType: 'json',
                data: formData,
                processData: false,
                contentType: false,
    ...
    function getTableActionBtnHtml(full) {
        ...
        html += '<button type="button" class="btn btn-default btn-edit" value="' + full["bay_to_route_id"] + '" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>';
        if (full["status"] == 1) {
            html += '<button type="button" class="btn btn-success btn-deactivate" value="' + full["bay_to_route_id"] + '" data-toggle="tooltip" title="Record is active"><i class="fa fa-check-circle" aria-hidden="true"></i></button>';
        } else {
            html += '<button type="button" class="btn btn-warning btn-activate" value="' + full["bay_to_route_id"] + '" data-toggle="tooltip" title="Record is inactive"><i class="fa fa-minus-circle" aria-hidden="true"></i></button>';
        }
        html += '</div>';
        return html;
    }
...
//        resetMasterDataForm();
        ...
        jQuery.post(site_url + '/bus_bay_route/getRecordById', param, function (response) {

            if (response !== null) {
                if (response.status == "1") {
                    var data = response.data
                    jQuery("#id").val(data.bay_to_route_id);
                    jQuery("#bay_id").val(data.bay_id);
    ...
    function deactivateById(id) {
        var param = {};
        param.id = id;
        jQuery.post(site_url + '/bus_bay_route/hide', param, function (response) {
            if (response !== null) {
                if (response.status == "1") {
...
//            swal("Delete failed!", "Deactivation failed.", "error");
    ...
    function activateById(id) {
        var param = {};
        param.id = id;
        jQuery.post(site_url + '/bus_bay_route/show', param, function (response) {
            if (response !== null) {
                if (response.status == "1") {
