<script>
    ...
    jQuery(document).ready(function () {
        masterdata_form_validator = jQuery("#location_form").validate({
            rules: {
                location_id: {
                    required: true
                }, location_name: {
        ...
        var_data_grid_table = jQuery('#data_grid_table').DataTable({
            ...
            "ajax": {
                "url": site_url + "/location/getDatatableRecords",
                "type": "POST"
            },
            "columns": [
			{"data": "location_name"}, 
			{"data": "location_name_si"}, 
...
// --------------------- END OF DOCUMENT READY CODE ----------------------------
    ...
    jQuery(document).on('click', '#btn_save', function (event) {
        var is_valid = jQuery("#location_form").valid();
        if (is_valid) {
            save_form();
        } else {
...
// --------------------- END OF EVENT HANDLING CODE ----------------------------
    ...
    function save_form() {
        swal({
            ...
            var data_form = document.getElementById('location_form');
            var formData = new FormData(data_form);
            jQuery.ajax({
                method: 'POST',
                url: site_url + '/location/save',
                dataType: 'json',
                data: formData,
                processData: false,
                contentType: false,
    ...
    function getTableActionBtnHtml(full) {
        ...
        html += '<button type="button" class="btn btn-default btn-edit" value="' + full["location_id"] + '" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>';
        if (full["status"] == 1) {
            html += '<button type="button" class="btn btn-success btn-deactivate" value="' + full["location_id"] + '" data-toggle="tooltip" title="Record is active"><i class="fa fa-check-circle" aria-hidden="true"></i></button>';
        } else {
            html += '<button type="button" class="btn btn-warning btn-activate" value="' + full["location_id"] + '" data-toggle="tooltip" title="Record is inactive"><i class="fa fa-minus-circle" aria-hidden="true"></i></button>';
        }
        html += '</div>';
        return html;
    }
...
//        resetMasterDataForm();
        ...
        jQuery.post(site_url + '/location/getRecordById', param, function (response) {

            if (response !== null) {
                if (response.status == "1") {
                    var data = response.data
                    jQuery("#id").val(data.location_id);
                    jQuery("#location_name").val(data.location_name);
                    jQuery("#location_name_si").val(data.location_name_si);
                    jQuery("#location_name_ta").val(data.location_name_ta);
    ...
    function deactivateById(id) {
        var param = {};
        param.id = id;
        jQuery.post(site_url + '/location/hide', param, function (response) {
            if (response !== null) {
                if (response.status == "1") {
...
//            swal("Delete failed!", "Deactivation failed.", "error");
    ...
    function activateById(id) {
        var param = {};
        param.id = id;
        jQuery.post(site_url + '/location/show', param, function (response) {
            if (response !== null) {
                if (response.status == "1") {
