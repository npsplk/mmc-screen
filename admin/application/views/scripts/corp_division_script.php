<script>
    ...
    var var_masterdata_table;
    jQuery(document).ready(function () {
        masterdata_form_validator = jQuery("#masterdata_form").validate({
            rules: {
                division_name: {
                    valid_name: true,
                },
        ...
        var_masterdata_table = jQuery('#masterdata_table').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": site_url + "/corp_division/getDatatableRecords",
                "type": "POST"
            },
            "columns": [
                {"data": "division_name"},
        ...
        jQuery('#masterdata_table').on('error.dt', function (e, settings, techNote, message) {
...
// --------------------- END OF DOCUMENT READY CODE ----------------------------
    ...
    jQuery(document).on('click', '#btn_save', function (event) {
        var is_valid = jQuery("#masterdata_form").valid();
        if (is_valid) {
            save_form();
        } else {
...
// --------------------- END OF EVENT HANDLING CODE ----------------------------
    ...
    function save_form() {
        swal({
            ...
            var formData = new FormData(masterdata_form);
            jQuery.ajax({
                method: 'POST',
                url: site_url + '/corp_division/save',
                dataType: 'json',
                data: formData,
                processData: false,
                contentType: false,
                ...
                if (data.status == '1') {
                    ...
                    var_masterdata_table.ajax.reload();
                    resetMasterDataForm();
                } else {
    ...
    function getTableActionBtnHtml(full) {
        ...
        html += '<button type="button" class="btn btn-default btn-edit" value="' + full["division_id"] + '" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>';
        if (full["status"] == 1) {
            html += '<button type="button" class="btn btn-success btn-deactivate" value="' + full["division_id"] + '" data-toggle="tooltip" title="Record is active"><i class="fa fa-check-circle" aria-hidden="true"></i></button>';
        } else {
            html += '<button type="button" class="btn btn-warning btn-activate" value="' + full["division_id"] + '" data-toggle="tooltip" title="Record is inactive"><i class="fa fa-minus-circle" aria-hidden="true"></i></button>';
        }
        html += '</div>';
        return html;
    }
    ...
    function getRecordById(id) {
        ...
        jQuery.post(site_url + '/corp_division/getRecordById', param, function (response) {

            if (response !== null) {
                if (response.status == "1") {
                    var data = response.data

                    jQuery("#division_name").val(data.division_name);
                    jQuery("#division_code").val(data.division_code);
                    jQuery("#id").val(data.division_id);
    ...
    function deactivateById(id) {
        var param = {};
        param.id = id;
        jQuery.post(site_url + '/corp_division/hide', param, function (response) {
            if (response !== null) {
                if (response.status == "1") {
//                swal("Deleted!", "User account deleted.", "success");
                    var_masterdata_table.ajax.reload();
                } else {
                    swal("failed!", "User account delete failed.", "error");
                }
            } else {
//            swal("Delete failed!", "User account delete failed.", "error");
    ...
    function activateById(id) {
        var param = {};
        param.id = id;
        jQuery.post(site_url + '/corp_division/show', param, function (response) {
            if (response !== null) {
                if (response.status == "1") {
//                swal("Deleted!", "User account deleted.", "success");
                    var_masterdata_table.ajax.reload();
