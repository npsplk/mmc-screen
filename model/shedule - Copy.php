...
// get configuration 
	 ...
	 $current_time= date("H:i");
	  ...
	  $sql="SELECT S.*, R.route, R.to_location, R.to_location_si, R.to_location_ta FROM mmc_bus_shedule AS S
	 LEFT JOIN mmc_master_route AS R ON S.route_id=R.route_id
	  WHERE S.route_id =$route_id AND week_day_id=$integer_date AND departure_time> '$current_time' ORDER BY departure_time ASC LIMIT 0, 12";
...
function getRouteDetail($bay_id, $db_conn){
		   $sql="SELECT route_id FROM mmc_master_bay 
	  WHERE bay_id =$bay_id";
